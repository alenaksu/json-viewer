...
export function generateNodePreview(node, options) {
  ...
  for (const key of keys) {
    ...
    if (nodeType === TYPE_OBJECT) nodePreview.push('{ ... }');else if (nodeType === TYPE_ARRAY) nodePreview.push('[ ... ]');else if (nodeType === TYPE_STRING) nodePreview.push(`"${nodeValue.substring(0, maxLength)}${nodeValue.length > maxLength ? '...' : ''}"`);else nodePreview.push(String(nodeValue));
