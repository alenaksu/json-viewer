 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */let $e;function me(s){return(t,e)=>ge(t,e,{get(){return(this.renderRoot??$e??($e=document.createDocumentFragment())).querySelectorAll(s)}})}/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
...
`;var Te=Object.defineProperty,Y=(s,t,e,i)=>{for(var r=void 0,n=s.length-1,o;n>=0;n--)(o=s[n])&&(r=o(t,e,r)||r);return r&&Te(t,e,r),r},z,O,C,E,qt,at;const $t=class $t extends L{constructor(){super();T(this,E);T(this,z);T(this,O);T(this,C);this.state={expanded:{},filtered:{},highlight:null},this.lastFocusedItem=null,B(this,z,e=>i=>{i.preventDefault(),this.setState(Ct(e))}),B(this,O,e=>{const i=e.target;e.target===this&&F(this,E,at).call(this,this.lastFocusedItem||this.nodeElements[0]),i.matches('[role="treeitem"]')&&(this.lastFocusedItem&&(this.lastFocusedItem.tabIndex=-1),this.lastFocusedItem=i,this.tabIndex=-1,i.tabIndex=0)}),B(this,C,e=>{const i=e.relatedTarget;(!i||!this.contains(i))&&(this.tabIndex=0)}),this.addEventListener("focusin",w(this,O)),this.addEventListener("focusout",w(this,C))}static customRenderer(e,i){return JSON.stringify(e)}async setState(e){const i=this.state;this.state={...i,...e(i,this)}}connectedCallback(){!this.hasAttribute("data")&&!ot(this.data)&&this.setAttribute("data",this.innerText),this.setAttribute("role","node"),this.setAttribute("tabindex","0"),super.connectedCallback()}expand(e){this.setState(V(e,!0))}expandAll(){this.setState(V("**",!0))}collapseAll(){this.setState(V("**",!1))}collapse(e){this.setState(V(e,!1))}*search(e){for(const[i,r]of ft(this.data))k(i)&&String(i).match(e)&&(this.expand(r),this.updateComplete.then(()=>{const n=this.shadowRoot.querySelector(`[data-path="${r}"]`);n.scrollIntoView({behavior:"smooth",inline:"center",block:"center"}),n.focus()}),this.setState(Nt(r)),yield{value:i,path:r});this.setState(Nt(null))}filter(e){this.setState(Oe(e))}resetFilter(){this.setState(Ce())}renderObject(e,i){return v`
            <ul part="object" role="group">
                ${be(Object.entries(e),([r,n])=>{const o=i?`${i}.${r}`:r,l=k(n),a=this.state.expanded[o];return this.state.filtered[o]?u:v`
                              ...
                              >
                                  ...
                                  >
                                      ${r}:
                                      ${st(!l&&!a,()=>this.renderNodePreview(n))}
                                  </span>
        ...
        `}renderValue(e,i=""){return k(e)?this.renderPrimitive(e,i):this.renderObject(e,i)}renderNodePreview(e){return v`<span part="preview" class="preview"> ${Ee(e)} </span>`}renderPrimitive(e,i){const r=this.state.highlight,n=nt(e),o=this.constructor.customRenderer(e,i),l=v`
        ...
        `}};z=new WeakMap,O=new WeakMap,C=new WeakMap,E=new WeakSet,qt=function(e){if(!["ArrowDown","ArrowUp","ArrowRight","ArrowLeft","Home","End"].includes(e.key))return;const i=[...this.nodeElements],r=this.matches(":dir(ltr)"),n=this.matches(":dir(rtl)");if(i.length>0){e.preventDefault();const o=i.findIndex(p=>p.matches(":focus")),l=i[o],a=this.state.expanded[l.dataset.path],d=k(xe(this.data,l.dataset.path)),c=p=>{const f=i[Math.max(Math.min(p,i.length-1),0)];F(this,E,at).call(this,f)},h=p=>{this.setState(Ct(l.dataset.path,p))};e.key==="ArrowDown"?c(o+1):e.key==="ArrowUp"?c(o-1):r&&e.key==="ArrowRight"||n&&e.key==="ArrowLeft"?!l||a||d?c(o+1):h(!0):r&&e.key==="ArrowLeft"||n&&e.key==="ArrowRight"?!l||!a||d?c(o-1):h(!1):e.key==="Home"?c(0):e.key==="End"&&c(i.length-1)}},at=function(e){e.focus()},$t.styles=[Ne];let m=$t;Y([Ht({converter:Pe,type:Object})],m.prototype,"data");Y([zt()],m.prototype,"state");Y([zt()],m.prototype,"lastFocusedItem");Y([me('[role="treeitem"]')],m.prototype,"nodeElements");var rt;customElements.define("json-viewer",(rt=class extends m{static customRenderer(s){if(typeof s=="string"){if(URL.canParse(s))return v`<a href="${s}" target="_blank">${s}</a>`;if(Date.parse(s))return new Date(s).toLocaleString()}else if(typeof s=="number")return s.toFixed(2);return super.customRenderer(s)}},yt(rt,"styles",[m.styles,Rt`
                a {
                    color: white;
                    text-decoration: underline;
                }
            `]),rt));const Bt=Comlink.wrap(new Worker(new URL("/json-viewer/assets/worker-Cf5OLqXr.js",import.meta.url))),Ue=document.querySelector("#json"),S=document.querySelector("json-viewer"),ke=document.querySelector("#toggle-panel"),Ft=document.querySelector("#container"),Tt=document.querySelector("#loader"),Re=document.querySelector("#expand"),je=document.querySelector("#collapse"),it=document.querySelector("#filter"),lt=document.querySelector("#search");let ct;Re.addEventListener("click",s=>{s.preventDefault(),S.expandAll()});je.addEventListener("click",s=>{s.preventDefault(),S.collapseAll()});it.addEventListener("change",()=>{it.value?S.filter(it.value):S.resetFilter()});lt.addEventListener("input",()=>{ct=S.search(lt.value)});lt.addEventListener("keyup",s=>{ct&&s.keyCode===13&&ct.next()});const Le=(s,t=500)=>{let e;return(...i)=>{clearTimeout(e),e=setTimeout(()=>s(...i),t)}},Me=s=>{try{S.data=JSON.parse(s)}catch(t){S.data=t.message}},Vt=()=>{const s=gt.getValue();Tt.hidden=!1,Promise.all([Me(s),Bt.crush(s).then(t=>{location.hash=t})]).then(()=>{Tt.hidden=!0})},gt=CodeMirror.fromTextArea(Ue,{mode:{name:"javascript",json:!0},lineNumbers:!0,theme:"jsv",styleActiveLine:!0,lint:{esversion:6}});ke.addEventListener("click",()=>{Ft.classList.toggle("collapsed")});gt.on("change",Le(Vt));Bt.uncrush(location.hash.slice(1)).then(s=>{s?(gt.setValue(s),Ft.classList.add("collapsed")):Vt()});
