...
`),function(...e){return e.filter(Boolean).join(" ")}(r&&"key",e&&"collapsable",t&&"collapsableCollapsed"),n,r);class I extends T{constructor(...e){super(...e),this.data=null,this.collapsed=!0,this.handleKeyClick=e=>{e.preventDefault(),this.collapsed=!this.collapsed}}static get properties(){return{data:W,collapsed:Boolean,key:String}}static get is(){return"json-nested-object-node"}renderValue(e){return B(e)?e:M(F||(F=q`
                  <span class=${0}>${0}</span>
              `),P(e),JSON.stringify(e))}renderChild(e){return this.collapsed?M(z||(z=q`
                  <span class="preview">
                      ${0}
                  </span>
              `),function(e,t){const{nodeCount:n,maxLength:r}={nodeCount:3,maxLength:15,...t},o=Array.isArray(e),s=Object.keys(e),i=s.slice(0,n),a=[o?"[ ":"{ "];return a.push(i.reduce((t,n)=>{const s=[],i=e[n],a=P(i);return o||s.push(`${n}: `),"object"===a?s.push("{ ... }"):"array"===a?s.push("[ ... ]"):"string"===a?s.push(`"${i.substring(0,r)}${i.length>r?"...":""}"`):s.push(String(i)),t.concat(s.join(""))},[]).join(", ")),s.length>n&&a.push(", ..."),a.push(o?" ]":" }"),a.join("")}(e)):M(H||(H=q`
                  <json-object-node data=${0}></json-object-node>
              `),e)}render(){const{data:e,key:t}=this,n=R(e)||B(e);return M(K||(K=q`
            ${0}
            ${0}
        `),G({isCollapsable:!n,collapsed:this.collapsed,key:t,onClick:!n&&this.handleKeyClick}),n?this.renderValue(e):this.renderChild(e))}}class Q extends T{constructor(...e){super(...e),this.data=null,this.collapsed=!0}static get is(){return"json-object-node"}static get properties(){return{data:W,collapsed:Boolean}}renderObject(e){return Object.keys(e).map(t=>M(V||(V=q`
                <li>
                    <json-nested-object-node key=${0} data=${0}></json-nested-object-node>
                </li>
            `),t,e[t]).withKey(t))}renderPrimitive(e){return void 0!==e?M(J||(J=q`
                  <li>${0}</li>
              `),e):null}render(){const{data:e}=this;return M(X||(X=q`
            <ul>
                ${0}
            </ul>
        `),R(e)?this.renderPrimitive(e):this.renderObject(e))}}class U extends T{constructor(...e){super(...e),this.data=null}static get is(){return"json-viewer"}static get properties(){return{data:W}}connectedCallback(){if(!this.hasAttribute("data")){const e=this.innerText.trim();e&&(this.data=JSON.parse(e))}this.attachShadow({mode:"open"}),super.connectedCallback()}render(){return M(Z||(Z=q`
            <style>
                :host {
            ...
            <json-object-node data=${0}></json-object-node>
        `),this.data)}}customElements.define(Q.is,Q),customElements.define(I.is,I),customElements.define(U.is,U);
